...
(module
	...
	(func (export "call")
		...
		;; Load a storage value and skip output
		(call $assert
			(i32.eq
				(call $seal_get_storage
					(i32.const 0)		;; The pointer to the storage key to fetch
					(i32.const -1)		;; Pointer to the output buffer (sentinel value = skip)
		...
		;; Output buffer size should stay at 4k because it was not overwritten with the real size
		(call $assert
			(i32.eq
